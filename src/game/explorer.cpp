...
static const SceneInfo SCENE_TABLE[] = {
    ...
    { 1887,  2, {{MASK_R,   63}, {MASK_R, 104}}, "Vestibule" },

    // Tentacles & Halberd
    { 2353,  6, {{MASK_B,   79}, {MASK_U,  140}, {MASK_R, 188},
                 {MASK_D, 226}, {MASK_L, 292}, {MASK_U, 343}}, "Tentacles" },

    // Snake Room (snake_room — tentacle_room alternate, row 7 col 2)
    { 3097,  4, {{MASK_B, 55}, {MASK_B,   98}, {MASK_B, 137}, {MASK_U, 159}}, "Snake Room" },

    // Fire Pit (ropes)
    { 3561,  4, {{MASK_R,  108}, {MASK_R, 147}, {MASK_R, 201}, {MASK_R, 245}}, "Fire Pit" },

    // Yellow Brick Road
    { 4139,  9, {{MASK_L,   36}, {MASK_U, 75}, {MASK_R, 131}, {MASK_U, 174},
                 {MASK_L, 227}, {MASK_U,  283}, {MASK_B, 363}, {MASK_R, 400},
                 {MASK_U,  441}}, "Yellow Brick Road" },

    // Bubbling Cauldron / Acid Creature (bubbling_cauldron — tentacle_room row 7 col 3)
    { 5123,  5, {{MASK_U,   56}, {MASK_B, 113}, {MASK_D, 151},
                 {MASK_B, 183}, {MASK_R, 253}}, "Bubbling Cauldron" },
    ...
    { 5683,  4, {{MASK_B, 64}, {MASK_R, 109}, {MASK_B, 163}, {MASK_U, 201}}, "Giddy Goons" },

    // YMCA Room (flattening stairs)
    { 6375,  4, {{MASK_L, 52}, {MASK_B, 113}, {MASK_L, 146}, {MASK_L, 213}}, "YMCA Room" },

    // Forge (Smithee — smithee, row 6 col 1)
    // Verified: human run 20260420. Slot 5 B@374: bot fired at 368 (pre-window), missed.
    { 6994,  5, {{MASK_B,   62}, {MASK_B, 127}, {MASK_L, 173},
    ...
    { 9529,  4, {{MASK_R, 76}, {MASK_L,  117}, {MASK_D, 165}, {MASK_L, 213}}, "Fire Room" },

    // Flying Barding
    { 10021, 6, {{MASK_R,   99}, {MASK_L, 134}, {MASK_R, 178}, {MASK_L, 225},
                 {MASK_L, 273}, {MASK_L, 308}}, "Flying Barding" },

    // Chapel (Robot Knight / checkered floor)
    { 10741, 8, {{MASK_R,   85}, {MASK_L,  123}, {MASK_U, 164}, {MASK_L, 214},
                 {MASK_R, 260}, {MASK_L, 290}, {MASK_R, 314}, {MASK_B, 371}}, "Chapel" },

    // Mausoleum (Crypt Creeps)
    { 11489, 6, {{MASK_U, 65}, {MASK_B, 101}, {MASK_U, 139},
                 {MASK_B,  181}, {MASK_L,  226}, {MASK_B, 262}}, "Mausoleum" },

    // Catwalk Bats (catwalk_bats — yellow_brick_road row 9 col 3)
    { 12190, 5, {{MASK_U,   47}, {MASK_U, 77}, {MASK_B, 144},
                 {MASK_R, 179}, {MASK_R,  211}}, "Catwalk Bats" },
    ...
    { 12725, 4, {{MASK_L,  110}, {MASK_L, 160}, {MASK_L, 208}, {MASK_L, 250}}, "Fire Pit Rev" },

    // Yellow Brick Road (reversed)
    { 13303, 9, {{MASK_R,   41}, {MASK_U, 74}, {MASK_L, 128}, {MASK_U, 172},
                 {MASK_R, 221}, {MASK_U, 278}, {MASK_B, 361}, {MASK_L, 399},
                 {MASK_U, 442}}, "Yellow Brick Rev" },
    ...
    { 15653, 5, {{MASK_B,   54}, {MASK_B, 131}, {MASK_R, 172},
                 {MASK_B, 241}, {MASK_B, 378}}, "Forge Rev" },

    // Flying Barding (reversed)
    { 16544, 6, {{MASK_L,   95}, {MASK_R,  132}, {MASK_L, 173}, {MASK_R, 223},
                 {MASK_R, 269}, {MASK_R, 304}}, "Flying Barding Rev" },
    ...
    { 18662, 6, {{MASK_U, 70}, {MASK_B, 99}, {MASK_U, 137},
    ...
    { 19520, 4, {{MASK_U,   90}, {MASK_B, 188}, {MASK_D, 252}, {MASK_U, 309}}, "Socker Boppers Rev" },
    ...
    { 20187, 3, {{MASK_D,   98}, {MASK_U,  133}, {MASK_L, 163}}, "Tilting Room" },

    // Throne Room (throne_room — row 11 col 1)
    // Levels.json move "U,R" is split into two ROM windows: U@78 then R@87.
    { 20674, 4, {{MASK_R, 56}, {MASK_U, 76}, {MASK_R,  87}, {MASK_R, 199}}, "Throne Room" },
    ...
    { 21212, 8, {{MASK_L, 93}, {MASK_R, 127}, {MASK_U, 167}, {MASK_R, 211},
    ...
    { 22936,13, {{MASK_L,   16}, {MASK_R, 55}, {MASK_L, 104}, {MASK_R, 151},
                 {MASK_U,  280}, {MASK_U, 346}, {MASK_U, 407}, {MASK_U, 473},
                 {MASK_R, 598}, {MASK_L, 673}, {MASK_R, 735}, {MASK_L, 797},
                 {MASK_R,  913}}, "Pirates" },
    ...
    { 24378, 9, {{MASK_B,  128}, {MASK_U,  217}, {MASK_U, 250}, {MASK_U, 301},
    ...
    { 25536, 3, {{MASK_L,   74}, {MASK_L, 168}, {MASK_R, 244}}, "Knight & Light" },

    // Boulder Trench (colored balls)
    { 26098, 7, {{MASK_D, 130}, {MASK_D, 180}, {MASK_D, 229}, {MASK_D, 277},
                 {MASK_D, 324}, {MASK_D, 368}, {MASK_U, 411}}, "Boulder Trench" },

    // Three Caves (jagged door / geyser)
    { 26778, 3, {{MASK_U,   58}, {MASK_U,  102}, {MASK_L, 236}}, "Three Caves" },

    // Dragon's Lair (final)
    { 28938,12, {{MASK_L, 107}, {MASK_L, 380}, {MASK_L, 578}, {MASK_L, 568},
                 {MASK_D, 1123}, {MASK_D, 1300},
                 {MASK_R, 1436}, {MASK_U, 1519},
                 {MASK_B, 1555}, {MASK_B, 1643}, {MASK_L, 1732}, {MASK_B, 1794}}, "Dragon's Lair" },
};
